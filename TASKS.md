...
- [ ] ⏳ 1.18 Component SKUs — `planner.component_skus` expands Send-to-Cart into component lines; quote/CSV/PDF breakdown; surfaced in audit tool (S6) [Sprint 2]. **Built 19 Aug 2026, commit `ade39f8`** (O cycle; reviewer fix-then-ship, all blocking fixes applied; `npm run build` ✓). **Verified-local only — NOT live.** Ships **dormant**: no product carries the metafield, and the no-op case was proven byte-identical (CSV, PDF text, on-screen total, cart `lines`) against the pre-change baseline. Component prices are included in the quote total so it matches what the cart charges; unresolvable variantIds show as unknown at $0.00 (never invented) and are flagged in the audit tool. **Two gates before this can go ✅:** (i) 🧠/👤 **A+H must confirm add-on vs bill-of-materials intent** — if `component_skus` was meant as a BOM the parent price already covers the parts and populating a metafield would overcharge (see [RELAY.md](RELAY.md) 1.18 entry, ruling 1); (ii) 👤 H sets the metafield **and exposes it to the Storefront API**, then live-verifies (runbook R4).
