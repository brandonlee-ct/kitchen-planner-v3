...
- [ ] ⬜ C8 1.18 follow-up code fixes — **gates gate (ii) of 1.18: both must be on `main` before H populates `planner.component_skus`.** (a) `parseComponentSkus` quantity edges: a fractional qty below 1 (`0.4`) is silently promoted to 1, a fractional qty above 1 (`2.7`) is silently floored, and a present-but-unreadable qty (`"two"`, `Infinity`) silently becomes 1 — all three invent a quantity, which is exactly what O ruling 4 forbids, and component prices now reach the customer total. (b) Audit-tool resolved-set: `runCatalogueAudit` resolves component variants against **all fetched nodes including `(Draft)` products**, while the runtime resolves against `products`, which drops drafts — so the audit can print `OK` for a component that will render as "Unknown component / $0.00" and that Shopify may reject, failing the WHOLE cart (1.18 risk 2). A falsely-reassuring audit is worse than no audit, and runbook R4 step 4 tells H to trust it.
