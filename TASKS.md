...
- [ ] ⬜ 1.15d Audit/parse alignment (follow-up from S2 reviewer) — `runCatalogueAudit` (S1) still uses `parseInt`; point it at `parseDimMm` so it can't false-report a Dimension-JSON metafield as "unparseable". Harmless today (no Dimension-JSON in catalogue); small brief. [pending A-audit ruling]
