...
- [x] ✅ 1.15a Catalogue audit tool — `?catalogaudit=1` read-only report of missing/unparseable `planner.*` metafields + applied fallbacks (S1). Reviewer PASS + build ✓, commit `1052d25`. **H live-verified 27–29 Jul** (used on live to produce Sprint-1 audit counts: 53 products / 40 drafts, 0 unparseable dims, 3 missing `glb_url`).
- [x] ✅ 1.15b Robust dimension parsing — integer/decimal/Dimension-JSON + unit conversion in `shopifyNodeToProduct` (`parseDimMm`); visible "default size" badge (S2). Reviewer PASS + build ✓, commit `b1b2d16`. **H live-verified** (badge visible in catalogue; live audit shows 0 unparseable dims). NOTE: Item 1's real cause is reused/stretched GLBs (data fix under 1.15c), so S2 is future-proofing + badge.
- [x] ✅ 1.15d Audit/parse alignment (follow-up from S2 reviewer) — `runCatalogueAudit` (S1) now uses `parseDimMm` so it can't false-report a Dimension-JSON/cm/m/inch metafield as "unparseable". Reviewer PASS-WITH-NITS + build ✓, commit `174c5dc`. **A re-check PASS; H live-verified** (audit counts read truthfully live).
...
**Item-10 smoke remainder — delegated to a tester (browser-only, no Git).** Desktop passed: Save, CSV+PDF, undo/redo, Send-to-Cart (service line). Still to verify (tester script in H's report): Restart Planner · cabinets on 300mm slab (place→save→reload) · power point in elevation · door/window select+drag with dims · zoom speed with cabinet selected · **all of the above on iPad/touch incl. long-press select.** Sprint 1 close is provisional until this returns (or H accepts partial close with these + the v1–v3 gap documented).

Polish / UX (non-blocking — O to write briefs when scheduled):
...
- [ ] ⬜ C3 **Bug:** Send-to-Cart button stuck on "Adding to cart…" after browser Back from checkout — needs a state reset.
- [ ] ⬜ C4 Signed-in state not obvious — make the 👤 icon bright green after sign-in.
- [ ] ⬜ C5 Fix the S5 "how to test" note (`S-BRIEFS-ITEMS-0-5.md`) — `serialiseScene()` isn't callable on prod (debug hook removed per Task Q); verify v4 via saved `scene_json` or `bbk_draft_autosave` after an edit. (Note corrected 29 Jul.)
